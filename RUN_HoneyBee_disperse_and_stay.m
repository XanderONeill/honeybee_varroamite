...
SP_p = drift;                   % Dispersal params [nu, H_0];
...
tspan = 0:1:12*n_years;

options = odeset('NonNegative', 1:5*N, 'RelTol', 1e-9, 'AbsTol', 1e-9);
% options = odeset('NonNegative', 1:5*N);
y0 = repmat([6, 14225, 10, 0, 0], 1, N);
[t,y] = ode45(@(t,y) HoneyBee_disperse_and_stay(t, y, BB_p, HB_p, VM_p, SP_p, types, type_params), tspan, y0, options);
...
% total honeybee population in general hive
H_T = y(1:n_years*12+1,5*N_G-3) + y(1:n_years*12+1,5*N_G-1) + y(1:n_years*12+1,5*N_G-0);

subplot(2,1,1)
% yyaxis left
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*N_G-4), 'k-.', 'LineWidth', 1.5) %brood in general hive
hold on
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*N_G-3), '-', 'LineWidth', 1.5, 'Color', [0.9290 0.6940 0.1250])  % total honeybees in general hive

xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96]);       xticklabels('');
...
% yyaxis right
% hold on
% plot(t(1:n_years*12+1), y(1:n_years*12+1,5*N_G-1), '-', 'LineWidth', 1.5, 'Color', [0.9290 0.6940 0.1250])
% plot(t(1:n_years*12+1), y(1:n_years*12+1,5*N_G-0), '-', 'LineWidth', 1.5, 'Color', [0.9290 0.6940 0.1250])
% xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96]);       xticklabels('');
% ylim([0 5.1e4]);    yticks([0 2e4 4e4]);    ylabel('Density per hive');
% 
% ax = gca;       ax.FontSize = 11;       set(gca,'box','off');
...
% ---- Varroa Densities ----%
yyaxis left
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*N_G-2), '-', 'LineWidth', 1.5, 'Color', [0.6350 0.0780 0.1840])

xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96]);
ylim([0 3000]);     yticks([0 1e3 2e3 3e3]);    ylabel('Mites');
ax = gca;           ax.FontSize = 11;           ax.YColor = [0.6350 0.0780 0.1840];

% ---- Varroa per hundred bee ----%
yyaxis right
plot(t(1:n_years*12+1), 100*y(1:n_years*12+1,5*N_G-2)./H_T, '--','LineWidth', 1.5, 'Color', [0 0.4470 0.7410])

xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96]);     xlabel('Time, t (months)');
...
% total honeybee population in tolerant hive
H_T = y(1:n_years*12+1,5*(N_G + N_T)-3) + y(1:n_years*12+1,5*(N_G + N_T)-1) + y(1:n_years*12+1,5*(N_G + N_T)-0);

subplot(2,1,1)
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*(N_G + N_T)-4), 'k-.', 'LineWidth', 1.5) %brood in tolerant hive
hold on
plot(t(1:n_years*12+1), H_T, '-', 'LineWidth', 1.5, 'Color', [0.9290 0.6940 0.1250])  % total honeybees in tolerant hive

xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96 108 120]);       xticklabels('');
ylim([0 5.1e4]);    yticks([0 2e4 4e4]);    ylabel('Density per hive');
...
% ---- Varroa Densities ----%
yyaxis left
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*(N_G + N_T)-2), '-', 'LineWidth', 1.5, 'Color', [0.6350 0.0780 0.1840])

xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96 108 120]);
ylim([0 9000]);     yticks([0 3e3 6e3 9e3]);    ylabel('Mites');
ax = gca;           ax.FontSize = 11;           ax.YColor = [0.6350 0.0780 0.1840];

% ---- Varroa per hundred bee ----%
yyaxis right
plot(t(1:n_years*12+1), 100*y(1:n_years*12+1,5*(N_G + N_T)-2)./H_T, '--','LineWidth', 1.5, 'Color', [0 0.4470 0.7410])

xlim([0 n_years*12]);     xticks([0 12 24 36 48 60 72 84 96 108 120]);     xlabel('Time, t (months)');
xtickangle(0);
...
% total honeybee population in resistant hive
H_T = y(1:n_years*12+1,5*(N_G + N_T + N_R)-3) + y(1:n_years*12+1,5*(N_G + N_T + N_R)-1) + y(1:n_years*12+1,5*(N_G + N_T + N_R)-0);

subplot(2,1,1)
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*(N_G + N_T + N_R)-4), 'k-.', 'LineWidth', 1.5) %brood in tolerant hive
hold on
plot(t(1:n_years*12+1), H_T, '-', 'LineWidth', 1.5, 'Color', [0.9290 0.6940 0.1250])  % total honeybees in tolerant hive
...
% ---- Varroa Densities ----%
yyaxis left
plot(t(1:n_years*12+1), y(1:n_years*12+1,5*(N_G + N_T + N_R)-2), '-', 'LineWidth', 1.5, 'Color', [0.6350 0.0780 0.1840])
% hold on
% plot(t(1:n_years*12+1), y(end-(n_years*12):end,5*(N_G + N_T + N_R)-2), '-.','LineWidth', 1, 'Color', [0.6350 0.0780 0.1840])
...
% ---- Varroa per hundred bee ----%
yyaxis right
plot(t(1:n_years*12+1), 100*y(1:n_years*12+1,5*(N_G + N_T + N_R)-2)./H_T, '--','LineWidth', 1.5, 'Color', [0 0.4470 0.7410])
% hold on
% plot(t(1:n_years*12+1), 100*y(end-(n_years*12):end,5*(N_G + N_T + N_R)-2)./H_T, ':','LineWidth', 1, 'Color', [0 0.4470 0.7410])
