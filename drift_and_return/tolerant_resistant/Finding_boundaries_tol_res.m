...
%%% Finding regions for EE, EP, and PP as seen in Figure 4, for a tolerant
%%% and resistant collective of hives 

%% Section 1: Parameters that are varied
...
%% Section 4: Mid Limit
% When looking at tolerant and resistant hives only this can be considered
% the upper limit. However, when considering all three populations, which
% gives three thresholds, this is the middle limit, and so I have kept this
% labelling for consistency

% the process follows the same idea as above. However, we know that the mid
% limit must be higher than the lower limit, and so j starts at the lower
% limit value instead of an arbitrary value (to speed things up).
...
%% Section 5: Upper Limit
% Only required when N_G, N_T > 0
...
save('Tol_20_res_limits')

%% Section 6: Figure production

close all 
clearvars
load('Tol_20_res_limits.mat')
